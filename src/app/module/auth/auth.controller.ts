...
export class AuthController {
     ...
     * /auth/sign-in:
     *   post:
     *     summary: Sign in or auto-register an user via Authentik
     *     description: Authenticates user through Authentik tokens. If user doesn't exist, it creates one automatically. Also restores soft-deleted users.
     *     tags: [Authentication]
     *     security:
     *       - bearerAuth: []
     *       - XSignatureAuth: []
    ...
    private signOut = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            ...
            await this.userService.DeleteAccessToken(metadata.id, accessToken, refreshToken);
